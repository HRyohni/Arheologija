...
def _export_map_image(iface, feature):
    ...
    # Set the size of the item using a QRectF. The units are millimeters.
    map_item.setRect(0, 0, 180, 100)

    # Set extent to the selected feature's bounding box with a buffer
    bbox = feature.geometry().boundingBox()
    bbox.scale(1.2)  # Add a 20% buffer around the feature
    map_item.setExtent(bbox)

    # Get the list of all layers from the QGIS canvas
    layers_to_render = iface.mapCanvas().layers()
    # Set the layers for the layout map item to render
    map_item.setLayers(layers_to_render)

    layout.addLayoutItem(map_item)

    # Set map item properties
...
def export_to_pdf(dlg, iface, selected_feature):
    ...
    try:
        ...
        # The temporary image file is only removed here, after the PDF is built,
        # which ensures it is available when ReportLab needs it.
